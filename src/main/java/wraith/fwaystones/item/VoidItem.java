...
public class VoidItem extends TexturedPolymerItem {

    protected boolean canTeleport = true;
    protected String translationName = "local_void";

    public VoidItem(Settings settings, Item polymerItem, String modelPath) {
        super(settings, polymerItem, modelPath);
    }


    public static String getBoundWaystone(ItemStack stack) {
        if (!(stack.getItem() instanceof VoidItem)) {
            return null;
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        ...
        tooltip.add(Text.translatable(
                ...
                Text.literal(name).styled(style ->
                        style.withColor(TextColor.parse(Text.translatable("fwaystones." + translationName + ".tooltip.arg_color").getString()).result().get())
                )
        ));
    }
}
